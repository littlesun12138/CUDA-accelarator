...
int main(int argc, char **argv) {
  ...
  float *dInput;  // The input 1D list
  float *dOutput; // The output list
  ...
  //@@ Allocate GPU memory
  cudaMalloc((void **) &dInput, numInputElements * sizeof(float));
  cudaMalloc((void **) &dOutput, numInputElements * sizeof(float));  

  //@@ Copy input memory to the GPU
  cudaMemcpy(dInput, hostInput, numInputElements * sizeof(float), cudaMemcpyHostToDevice);
  ...
  total<<<dimGrid, dimBlock>>>(dInput, dOutput, numInputElements);
  
  cudaDeviceSynchronize();  
  //@@ Copy the GPU output memory back to the CPU
  cudaMemcpy(hostOutput, dOutput, numOutputElements * sizeof(float), cudaMemcpyDeviceToHost);
